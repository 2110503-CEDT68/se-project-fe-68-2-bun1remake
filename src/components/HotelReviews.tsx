...
/* ─── schema helpers (backend has inconsistent field names) ─── */
...
function resolveUserId(c: CommentItem): string {
  ...
  const u = (c as unknown as Record<string, unknown>).user;
...
/* ─── ReviewCard with clamp-overflow detection ─────────────── */
    ...
    <article className="flex flex-col border border-[rgba(171,25,46,0.08)] bg-white p-4">
      <div className="flex items-start justify-between gap-2">
        <div className="flex items-center gap-2">
          ...
          <span className="font-figma-copy text-[1rem] text-[var(--figma-ink)]">{resolveUserName(c)}</span>
        </div>
        {canDel && (
          <button
            type="button"
      ...
      <p ref={ref} className="mt-2 line-clamp-3 font-figma-copy text-[1rem] leading-snug text-[var(--figma-ink)]">
        {getCommentText(c)}
      </p>
...
export default function HotelReviews({ hotelId }: HotelReviewsProps) {
  ...
  const recalcAverage = (list: CommentItem[]): number | null => {
  ...
  const canDelete = (c: CommentItem) =>
    userRole === "admin" || (userRole === "user" && !!userId && resolveUserId(c) === userId);
  ...
  const filtered = tab === "yours" ? comments.filter((c) => resolveUserId(c) === userId) : comments;
          ...
          <div className="relative w-full max-w-md bg-white p-6 shadow-xl">
            <div className="flex items-start justify-between gap-4">
              <div className="flex items-center gap-2">
                ...
                <span className="font-figma-copy text-[1.1rem] text-[var(--figma-ink)]">{resolveUserName(modal)}</span>
              </div>
              <div className="flex items-center gap-3">
                {canDelete(modal) && (
                  <button type="button" onClick={() => void handleDelete(modal._id)} className="text-[var(--figma-red)] opacity-70 hover:opacity-100" aria-label="Delete">
                    <TrashIcon />
                  </button>
                )}
            ...
            <p className="mt-4 font-figma-copy text-[1.05rem] leading-relaxed text-[var(--figma-ink)] whitespace-pre-wrap">{getCommentText(modal)}</p>
            <p className="mt-4 font-figma-copy text-[0.9rem] text-[var(--figma-ink-soft)]">{relativeTime(getDateField(modal))}</p>
          </div>
        </div>
      )}
      ...
      <section className="mt-8 border border-[rgba(171,25,46,0.08)] bg-[rgba(255,245,244,0.45)] p-5 sm:p-10">
        ...
        <div className="flex items-center justify-between gap-4">
          <h2 className="font-figma-copy text-[2rem] text-[var(--figma-ink)] sm:text-[2.5rem]">
            ...
            {averageRating !== null && (
              <span className="ml-2 text-[var(--figma-red)]">{averageRating.toFixed(1)}★</span>
            )}
          </h2>
          {token && (
            <button
              ...
              <div className="flex gap-1">
                {[1, 2, 3, 4, 5].map((v) => (
                  <button
                    ...
                    className={`text-[1.6rem] leading-none transition-colors ${v <= newRating ? "text-[var(--figma-red)]" : "text-[rgba(171,25,46,0.2)]"}`}
        ...
        {!isLoading && comments.length > 0 && (
          <div className="mt-5 flex gap-2">
            <button
              ...
              className={`px-4 py-1.5 font-figma-copy text-[1.05rem] transition-colors ${tab === "all" ? "bg-[var(--figma-red)] text-white" : "border border-[rgba(171,25,46,0.2)] text-[var(--figma-ink)] hover:bg-[rgba(171,25,46,0.05)]"}`}
            >
              All reviews
            </button>
            {token && (
              <button
                ...
                className={`px-4 py-1.5 font-figma-copy text-[1.05rem] transition-colors ${tab === "yours" ? "bg-[var(--figma-red)] text-white" : "border border-[rgba(171,25,46,0.2)] text-[var(--figma-ink)] hover:bg-[rgba(171,25,46,0.05)]"}`}
              >
                Your reviews
              </button>
            )}
        ...
        {isLoading ? (
          ...
          <>
            <div className="mt-5 grid gap-4 sm:grid-cols-2">
              {visible.map((c) => (
                <ReviewCard key={c._id} c={c} canDel={canDelete(c)} onDelete={(id) => void handleDelete(id)} onExpand={setModal} />
              ))}
            </div>
            {totalPages > 1 && (
              <div className="mt-6 flex items-center gap-3">
                <button type="button" onClick={() => setPage((p) => Math.max(1, p - 1))} disabled={page === 1} className="text-[var(--figma-red)] disabled:opacity-30" aria-label="Previous page">
                ...
                <button type="button" onClick={() => setPage((p) => Math.min(totalPages, p + 1))} disabled={page === totalPages} className="text-[var(--figma-red)] disabled:opacity-30" aria-label="Next page">
