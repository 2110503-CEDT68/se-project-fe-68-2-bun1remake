...
function userObj(c: CommentItem): UserLike | null {
  return (
    typeof c.userId === "object"
      ? c.userId
      : (c as unknown as { user?: unknown }).user
  ) as UserLike | null;
}
const userIdOf = (c: CommentItem) =>
  typeof c.userId === "string" ? c.userId : userObj(c)?._id || "";
const nameOf = (c: CommentItem) => userObj(c)?.name || "Guest";
const textOf = (c: CommentItem) =>
  c.comment || (c as unknown as { text?: string }).text || "";
const dateOf = (c: CommentItem) =>
  c.commentDate || (c as unknown as { createdAt?: string }).createdAt || "";
...
function ReviewCard({
  ...
  return (
    <article className="flex flex-col border border-[rgba(171,25,46,0.08)] bg-white p-4">
      ...
      <p className="mt-2 line-clamp-3 font-figma-copy text-[1rem] leading-snug text-[var(--figma-ink)] whitespace-pre-wrap">
        {textOf(c)}
      </p>
  ...
  return (
    <>
      ...
      <section className="mt-8 border border-[rgba(171,25,46,0.08)] bg-[rgba(255,245,244,0.45)] p-5 sm:p-10">
        {/* HEADER SECTION: Displays review count, average rating, and the "Toggle Form" button */}
        <div className="flex items-center justify-between gap-4">
          ...
          {/* ACTION BUTTON: Toggles the review form. Only visible to logged-in users (token check) */}
          {token && (
            <button
              type="button"
              onClick={() => setShowForm((v) => !v)}
              className="flex h-9 w-9 shrink-0 items-center justify-center bg-[var(--figma-red)] text-white font-figma-copy text-[1.4rem] leading-none"
        ...
        {/* REVIEW SUBMISSION FORM: Conditionally rendered when showForm is true */}
        {showForm && token && (
          <div className="mt-5 space-y-4 pt-5 bg-[#FDF1E8]/40 p-8 border-y border-[#AB192E]/10 space-y-10">
            ...
            {/* COMMENT TEXTAREA: Bound to 'text' state */}
            <textarea
              value={text}
              onChange={(e) => setText(e.target.value)}
              placeholder="Add Your Comment Here"
              rows={4}
              className="w-full resize-none bg-[#FDF1E8]/40 p-8 bg-[#FFAAAA]/15 shadow-[inset_4px_4px_4px_0_rgba(0,0,0,0.15),inset_-4px_-4px_4px_0_rgba(255,255,255,0.15)] border border-[#AB192E]/10 space-y-10 font-figma-copy text-[1.05rem] text-[var(--figma-ink)] placeholder:italic placeholder:text-[#FFAAAA] focus:outline-none"
            />
            {/* NOTIFICATION LAYER: Shows success/error messages from the 'notice' state */}
            <DismissibleNotice notice={notice} onClose={dismissNotice} />
            {/* SUBMIT BUTTON: Disables while the 'submitting' state is true to prevent double posts */}
            <button
              type="button"
              onClick={() => void submit()}
              disabled={submitting}
        ...
        {/* TAB NAVIGATION: Filter between "All" and "Yours". Only shows if reviews exist. */}
        ...
        {/* CONTENT AREA: Handles Loading, Empty States, and the Review List */}
        {loading ? (
          ...
          <>
            {/* LIST OF REVIEWS: Responsive grid showing the 'visible' slice of reviews for the current page */}
            ...
            {/* PAGINATION CONTROLS: Only visible if there's more than one page of content */}
            {totalPages > 1 && (
        ...
        {/* CALL TO ACTION: Shown to guests prompts them to log in to contribute */}
        {!token && (
